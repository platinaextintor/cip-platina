...
const importWorkspaceInput = document.querySelector("#importWorkspace");
...
document.querySelector("#exportWorkspace").addEventListener("click", exportWorkspace);
importWorkspaceInput.addEventListener("change", importWorkspace);
...
function projectPayload() {
  return {
    project: {
      name: "CIP - Central Inteligente de Processos",
      company: "Platina Extintores",
...
function exportWorkspace() {
  const payload = JSON.stringify(projectPayload(), null, 2);
  const blob = new Blob([payload], { type: "application/json" });
  const url = URL.createObjectURL(blob);
  const date = new Date().toISOString().slice(0, 10);
  const link = document.createElement("a");
  link.href = url;
  link.download = `cip-platina-dados-${date}.json`;
  document.body.appendChild(link);
  link.click();
  link.remove();
  URL.revokeObjectURL(url);
  saveStateNode.textContent = "backup JSON exportado";
}

function normalizeImportedWorkspace(payload) {
  const workspace = payload?.workspace || payload;
  const requiredLists = ["sectors", "roles", "processes", "tracks", "documents", "announcements"];
  if (!workspace || !requiredLists.every((key) => Array.isArray(workspace[key]))) {
    throw new Error("Arquivo JSON fora do formato da CIP.");
  }
  return workspace;
}
...
function importWorkspace(event) {
  ...
  reader.addEventListener("load", () => {
    try {
      state = normalizeImportedWorkspace(JSON.parse(reader.result));
      selected = { type: "company", id: "platina" };
      currentView = "mapa";
      persist();
      render();
      saveStateNode.textContent = `importado de ${file.name}`;
    } catch (error) {
